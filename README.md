...
DayMind 是一个本地完整可运行的极简每日状态复盘工具。它不是 Todo 软件，也不是 AI 聊天工具，而是把今日计划、今日记录、近期背景目标和最近 7 天历史收束成一份短而准确的今日状态报告。
...
3. 进入 `/app` 后编辑近期背景目标并保存。
4. 在今天添加几条计划，切换完成状态。
5. 写今日记录，等待自动保存。
6. 点击生成今日状态报告。
7. 切换历史日期，查看计划、记录、报告和背景目标快照。
8. 切换未来日期，提前添加未来计划。
